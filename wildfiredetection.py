# -*- coding: utf-8 -*-
"""WildfireDetection.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1zwNWGDxHK_cOadXgBD0Y8i1c93VfMCFf
"""
...
humidity = ncep.pick_bands('min_specific_humidity')
humidity_mask = (humidity > 75)
...
nir = modis.pick_bands('nir')
swir = modis.pick_bands('swir1')
burnedarea = (nir - swir) / (nir + swir)

goes = wf.ImageCollection.from_id('goes17:fulldisk:v1', start_datetime=start_datetimel5, end_datetime=end_datetimel5).mosaic()
#this line chooses the specific type of data it needs from the dataset (weather dataset, precipitation specifically)
ozone = goes.pick_bands('lwir0')
co2 = goes.pick_bands('lwir4')

detectionmasks = unwanted_areas | humidity_mask | ndvi_mask
detectioncombined_data = humidity.concat_bands(ozone).concat_bands(co2).concat_bands(ndvi).concat_bands(burnedarea).concat_bands(normaltemp).concat_bands(abnormaltemp)
detectioncombined_data = detectioncombined_data.mask(detectionmasks)

#reads data for classifier
df = detectioncombined_data
values = df.values[:, :]
X = values[:, :-1]
y = values[:, -1]
...
res = detectioncombined_data.compute(dl.scenes.AOI(united_states,crs='EP5G:4326', resolution = 0.5))
